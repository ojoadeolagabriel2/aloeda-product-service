...
SERVICE_NAME="${ENV_SERVICE_NAME:-aloeda-product-service}"
...
# build latest image
docker build -t "${IMAGE_NAME}:latest" -f "$DIR"/deployment/Dockerfile .
# build versioned image
docker build -t "${IMAGE_NAME}:${VERSION}" -f "$DIR"/deployment/Dockerfile .
# push
